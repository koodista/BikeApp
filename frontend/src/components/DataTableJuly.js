...
const DataTableJuly = () => {
  const [bikeJourneysJuly, setBikeJourneysJuly] = useState([]);
  ...
  const fetchData = async (page, limit) => {
    ...
    const response = await axios.get(
      `/api/bikejourneysjuly?page=${page}&limit=${limit}&delay=1`
    );
    setBikeJourneysJuly(response.data.data);
  ...
  // const handleFilter = () => {
  //   const filteredItems = bikeJourneysJuly.filter(
  //     (item) =>
  ...
  return (
    <div style={{ height: "100px" }}>
      <DataTable
        ...
        paginationRowsPerPageOptions={[10, 30, 50, 100, 500, 5000, 19411]}
